-- ACH-first payment cohort retention
-- Cohort: customers whose FIRST payment was an ACH payment (type='payment')
-- Cell: % of cohort who made an ACH payment in month M+N
WITH params AS (
    SELECT TO_DATE('2025-01-01') AS start_date,
           TO_DATE('2026-06-01') AS end_date  -- exclusive
),
first_time AS (
...
valid_customers AS (
    ...
    JOIN first_time ft
      ON fm.customer_id = ft.customer_id
