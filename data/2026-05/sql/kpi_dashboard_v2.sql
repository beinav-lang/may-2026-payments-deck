...
first_active AS (
  SELECT lease_id, first_active_date, dbt_first_invoice_date, dbtenant,
    DATE_TRUNC('month', CASE WHEN EXTRACT(DAY FROM first_active_date)>=25 THEN DATEADD(month,1,first_active_date) ELSE first_active_date END) AS LeaseActivationMonth,
    CASE WHEN DATEDIFF('day', dbt_first_invoice_date, first_active_date) < 90 THEN 'Onboarding' ELSE 'Mature' END AS cohort_status
  FROM (
    SELECT lds.lease_id, lds.snapshot_date AS first_active_date, lds.dbt_first_invoice_date, lds.dbtenant,
